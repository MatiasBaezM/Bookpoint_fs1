...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidTransition(TransicionEstadoInvalidaException ex, HttpServletRequest request) {
        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
    ...
    public ResponseEntity<ErrorResponse> handleOrigenNoDisponible(OrigenNoDisponibleException ex, HttpServletRequest request) {
        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.CONFLICT.value())
                .error("Conflict - Origen No Disponible")
                .message(ex.getMessage())
                .path(request.getRequestURI())
                .build();
